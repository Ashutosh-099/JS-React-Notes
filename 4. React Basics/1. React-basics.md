...
## Different ways to add images in the React application:
### Import image using ES6 modules-
- Directly import the image from the asset(assume it would be in asset folder) folder, common approach for the import.
```
...
### Using public folder:
- `public` folder is very useful, it is directly accessible in any file or in any component. We do need to import the files inside public folder. If we have image having large size, we can use public folder.
- Use `process.env.PUBLIC_URL` to get relative path of public folder.
```
const App = () => {
  return(
    <div>
      <img src={process.env.PUBLIC_URL + '/myImage.jpg'} / >
    </div>
  );
}
```

### Loading images from remote source:
- We can import image from external environment, just get the URL or API of the image which is available outside and put in src attribute.
```
const App = () => {
  return(
    <div>
      <img src="https://imageexample.com/myImage.jpg" />
    </div>
  );
}
```
